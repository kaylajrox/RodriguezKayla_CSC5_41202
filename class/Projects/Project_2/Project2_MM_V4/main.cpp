...
for(int n=1;n<=limit;n++){
    input(color,order,4,options,search);
...
char compic(char options[]){
  do{
    for (int i=0;i<4;i++){
        int j=rand()%8;
        cout<<options[j]<<" "; 
        }cout<<endl;
    }while (options[0]==options[1]||options[1]==options[2]||options[2]==options[3]);
