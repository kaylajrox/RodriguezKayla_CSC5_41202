 ...
 * Questions/Concerns:
2D array for table?
Fix check Function. I need it to repeat only if false, not working right
toupper used in search?-->verify if you need another options array
...
for(int n=1;n<=limit;n++){
    search=check(options,SIZE,color);
    input(color,order,SIZE,options,search);
...
/******************************************************************************/
void input(char color[],string order[],const int SIZE,char options[],int search){
   for (int i=0;i<SIZE;i++){
        do{
